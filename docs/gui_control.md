...
- `material_studio_gui_activate`: brings the existing Materials Studio window forward. If project context is omitted, it resolves the latest current structured project when available and activates that wrapper window. With `take_snapshot=true`, it captures the activated target window and, when project/revision context is available, refreshes the structured `report.json`, `view_audit.json`, view bundle, and `gui_current_revision` receipt. A successful capture returns `snapshot_status="captured"`, `snapshot_captured=true`, and a post-capture `window_management` receipt with `needs_snapshot=false`; the pre-capture recommendation remains under `window_management_before_capture` for audit evidence, so clients do not repeat the same snapshot. Independent single-window or dialog blockers are preserved. If another app takes foreground focus after activation but before capture, the activation remains successful while the snapshot is returned as `deferred_before_capture`; no screenshot evidence is persisted, and the response includes an exact `snapshot_retry_tool`/`snapshot_retry_payload` bound to the same workspace, project, revision, and existing window.
...
- `material_studio_gui_open_structure`: opens an existing structure file only when an existing Materials Studio window is available. Its `hotload_transport` is `auto`, `loop`, or `dialog`. `auto` prefers a fresh, signed GUI loop bound to the exact PID/HWND/project/revision and falls back to the already-running window's File/Open dialog only before a job is enqueued. `loop` fails closed when that binding is not ready; `dialog` always uses the existing File/Open path. After loop enqueue, timeout or failure never falls back to dialog because the import may already have occurred. Neither path implicitly launches `MatStudio.exe`, and both refuse methods that may spawn another Materials Studio window. When `project_id` and `revision` are provided, the tool also persists the GUI-open artifact into the structured revision's `view_audit.json`, view bundle, `modeling_health`, `modeling_report`, and `report.json` by default. If `project_id` is provided and `revision` is omitted, it resolves that project's current revision; if `project_id` is omitted, it only syncs diagnostics when `structure_path` matches the latest current revision's planned structure.
- `material_studio_gui_loop_prepare`: writes one fixed `import_structure` MaterialsScript loop and signed queue for an exact verified wrapper. It sends no GUI input and cannot start the loop through `RunMatScript.bat`; the returned script must be run once from Script Library/User Menu in the exact target Materials Studio window.
- `material_studio_gui_loop_status`: reads the signed heartbeat, exclusive loop lock, revision state, queue state, and optional job receipt. Only `loop_ready=true` authorizes loop transport.
- `material_studio_gui_loop_stop`: publishes a signed stop request for the exact binding without terminating Materials Studio, closing the window, or deleting documents.
...
- `material_studio_gui_fit_to_view`: preview-first framing action for the exact current revision wrapper. Preview refreshes the single-process/window and installed `#SVViewer3d.xml`, then runs a read-only native Win32 probe in a short-lived helper process with a 30-second hard deadline. The helper starts from the exact package module with Python isolated/no-site mode inside a kill-on-close Windows Job Object; its module path, SHA-256, package version, and direct interpreter are verified by the parent. The probe binds the exact HWND/PID/title, active MDI document, its unique visible `CViewer3DCtrl`, and the unique `3D Viewer` toolbar; all nine live button indexes, numeric command IDs, styles, reviewed states, and the separator must match before Fit is offered. It deliberately avoids the Materials Studio 20.1 UIA descendant-tree scan, which can block indefinitely. Explicit execute rechecks the current project/revision, workspace provenance, single-window state, complete native target, foreground state, and registry SHA-256 immediately before sending only bounded `WM_COMMAND(33299)` to the exact existing wrapper; it then repeats the probe, captures before/after BMPs by default, persists the GUI action log/report under the revision, and proves the structure SHA-256 is unchanged. Probe timeout or mismatch fails closed with no GUI input; any failure after command dispatch is marked as a possible side effect and is never retried automatically. If Fit succeeds but the final snapshot fails, only the snapshot is retried. It never uses coordinates, keyboard modifiers, or launches another `MatStudio.exe` process. The result is mechanical evidence only; visual acceptance still requires a separate snapshot review or visual-confirmation path.
- `material_studio_live_modeling_request`, `material_studio_live_update_with_patch`, and `material_studio_gui_apply_current_revision` accept `fit_to_view_after_open`. The high-level natural-language entry recognizes an explicit framing phrase inside structural create/patch and session-level show-current/rollback/redo/restore requests, so it preserves the selected modeling or revision-history workflow instead of replacing it with display-only `fit_to_view`. An explicit reload/open/show/push of the current revision stays on `show_current`, while a pure framing request remains `fit_to_view` and does not rematerialize the structure. Preview records `deferred_until_execute` and performs no GUI input. Execute runs structure materialization first, releases the revision-execution lock, then holds one GUI artifact transaction across fresh current-revision preflight, same-window open, verified Fit-to-View, the final snapshot, and report publication. The open snapshot serves as pre-action evidence and the post-fit snapshot is the authoritative final GUI snapshot even when file mtimes tie. `completed=true` requires an executed action, unchanged structure hash, and bound final snapshot. A requested framing step that is blocked before command dispatch returns `partial_success=true`, preserves the generated structure/open evidence, and may supply an exact `material_studio_gui_fit_to_view` retry payload; retrying it must not rerun the model build or create another rollback/redo revision. Once command dispatch was attempted or a side effect is possible, automatic Fit retry is forbidden and the response routes to status/manual review. If Fit completed but only the final snapshot failed, retry only the exact snapshot payload.
- The same three tools accept `prepare_view_replay_after_open`. The high-level entry infers it only when the request explicitly asks to hot-load/open the model in Materials Studio and also requests a view subset or a normality check; explicit `false` overrides that inference. Preview returns `deferred_until_execute`, while execute first materializes and hot-loads the immutable revision in the existing window. Only after the GUI artifact/report transaction is released does the server check the current revision, persist `gui_view_replay_manifest.json`, and check current again after the controller returns. If another writer advances the project during preparation, the old manifest remains immutable historical evidence, but the response publishes no stale continuation and directs the client to `material_studio_live_project_status` for the new current revision. Successful responses keep the original `next_action_plan` as the modeling/calculation authority and publish the replay separately through `visual_diagnostics_next_action_plan`, `next_action_tracks`, and `coordinated_next_action_plan`. This preparation is preview-only: it sends no GUI input, creates no revision, accepts no camera result, and does not rewrite the modeling report that was atomically published with the hot-load. A preparation failure returns partial success plus an exact workspace-bound `material_studio_gui_prepare_view_replay` retry payload; the generated structure and same-window open remain valid and must not be rerun. `show_current`, rollback, redo, and restore preserve their existing revision semantics while using the same post-hot-load preparation stage.
...
is present, and the exact existing wrapper is selected, foreground, visible,
non-minimized, identity-verified, and the only Materials Studio window. The same
preview payload is preserved in `next_action_plan`, `gui_current_revision`, hot-load
preflight, and the compact MCP visible-followup contract. The recommendation is
suppressed for OpenGL/GDI capture limitations, stale revisions, inactive windows, or
accepted external visual evidence. Status never turns this preview recommendation
into GUI input; explicit `execution_mode="execute"` remains a separate confirmation.
...
### GUI-side hot-load loop boundary

The GUI loop is an independently implemented, fixed-operation transport
inspired by the GUI-context polling, atomic-rename, and heartbeat pattern in
[DrYe1109/MS-MCP's pinned loop](https://github.com/DrYe1109/MS-MCP/blob/991a1b3ab2ad985529fb645dc82f47528a2a1297/materialscript/mcp_loop_gui.pl#L4-L100)
and its
[GUI-context requirement](https://github.com/DrYe1109/MS-MCP/blob/991a1b3ab2ad985529fb645dc82f47528a2a1297/README.md#L82-L91).
It does not accept queued Perl or implement arbitrary `do $script`. Jobs are
HMAC-signed data envelopes for `import_structure` only, published through
staging/pending/running/done-or-failed atomic transitions. The loop and its
live-wrapper publication are bound to PID, HWND, workspace, project, expected
revision, target revision, active-document name, and the immutable structure
SHA-256. Windows revalidates PID/HWND before and after enqueue; the GUI context
checks Materials Studio `ActiveDocument` against signed current state.

Materials Studio 20.1 requires the generated loop to be started once in the
exact GUI window through Script Library/User Menu. `RunMatScript.bat` runs a
separate script process and is not evidence that the loop is attached to the
open GUI. After a fresh signed heartbeat makes `loop_ready=true`, explicit
execute/hot-load flows can update the same wrapper window across revisions.
If health fails before enqueue, `auto` may use the existing dialog path. Any
failure after enqueue is treated as potentially side-effecting and is never
retried through the dialog. The importer intentionally leaves prior documents
open to avoid discarding unsaved manual edits. See [gui_loop.md](gui_loop.md)
for setup, receipts, and recovery rules.
...
6. Open or apply the current revision in the GUI. The tool wraps generated structures
   ...
   `execution_mode="execute"` to run and hot-load. CASTEP result and relaxation
   tools use the same existing-window finalizer. Their optional
   `fit_to_view_after_open` action runs only after a successful result hot-load,
   and `prepare_view_replay_after_open` prepares a manifest for the promoted or
   recorded result revision only after the GUI artifact/report lock is released.
