...
- `material_studio_gui_fit_to_view`: preview-first framing action for the exact current revision wrapper. Preview refreshes the single-process/window, installed `#SVViewer3d.xml`, and fresh UIA checks without GUI input. Explicit execute invokes only the server-verified `cmdViewer3DFitToView` `InvokePattern` in the existing window, captures before/after BMPs by default, persists the GUI action log/report under the revision, and proves the structure SHA-256 is unchanged. It never uses coordinates, keyboard modifiers, or launches another `MatStudio.exe` process. The result is mechanical evidence only; visual acceptance still requires a separate snapshot review or visual-confirmation path.
- `material_studio_live_modeling_request`, `material_studio_live_update_with_patch`, and `material_studio_gui_apply_current_revision` accept `fit_to_view_after_open`. The high-level natural-language entry recognizes an explicit framing phrase inside structural create/patch and session-level show-current/rollback/redo/restore requests, so it preserves the selected modeling or revision-history workflow instead of replacing it with display-only `fit_to_view`. An explicit reload/open/show/push of the current revision stays on `show_current`, while a pure framing request remains `fit_to_view` and does not rematerialize the structure. Preview records `deferred_until_execute` and performs no GUI input. Execute runs structure materialization first, releases the revision-execution lock, then holds one GUI artifact transaction across fresh current-revision preflight, same-window open, verified Fit-to-View, the final snapshot, and report publication. The open snapshot serves as pre-action evidence and the post-fit snapshot is the authoritative final GUI snapshot even when file mtimes tie. `completed=true` requires an executed action, unchanged structure hash, and bound final snapshot. A blocked or failed requested framing step returns `partial_success=true`, preserves the generated structure/open evidence, and supplies an exact `material_studio_gui_fit_to_view` retry payload; retrying must not rerun the model build or create another rollback/redo revision.
