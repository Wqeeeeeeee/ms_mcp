...
- `material_studio_live_modeling_request`, `material_studio_live_update_with_patch`, and `material_studio_gui_apply_current_revision` accept `fit_to_view_after_open`. The high-level natural-language entry also recognizes an explicit framing phrase inside a structural create/patch request, so the structural plan is not replaced by the display-only `fit_to_view` workflow. Preview records `deferred_until_execute` and performs no GUI input. Execute runs structure materialization first, releases the revision-execution lock, then holds one GUI artifact transaction across fresh current-revision preflight, same-window open, verified Fit-to-View, the final snapshot, and report publication. The open snapshot serves as pre-action evidence and the post-fit snapshot is the authoritative final GUI snapshot even when file mtimes tie. `completed=true` requires an executed action and unchanged structure hash. A blocked or failed requested framing step returns `partial_success=true`, preserves the generated structure/open evidence, and supplies an exact `material_studio_gui_fit_to_view` retry payload; retrying must not rerun the model build.
