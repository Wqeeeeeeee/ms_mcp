...
ORCHESTRATION_PACKAGE = (
    ROOT / "src" / "material_studio_mcp_server" / "orchestration"
)
...
def test_orchestration_import_preserves_exact_public_tools_and_schema() -> None:
    script = r'''
import copy
import inspect
import json
...
module_name = "material_studio_mcp_server.orchestration"
...
import material_studio_mcp_server.orchestration
...
def test_orchestration_import_is_silent_and_file_side_effect_free(
    tmp_path: Path,
) -> None:
    before = _file_state(ORCHESTRATION_PACKAGE)
    completed = subprocess.run(
        [
            sys.executable,
            "-c",
    ...
    assert _file_state(ORCHESTRATION_PACKAGE) == before
